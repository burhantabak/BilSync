...
export default function CategoryDisclosure({title, categoryList, handleClick}){
      ...
      <Disclosure>
        ...
        <Disclosure.Panel className={`text-gray-900 flex flex-col`}>
            {categoryList.map((value, index) =>{
                console.log("Called");
                return <button onClick={()=>handleClick(value)} key={index} className='py-1 px-2 hover:bg-gray-200 rounded lg'>
                    <h1>{value}</h1>
                </button>
            })}
        </Disclosure.Panel>
